...
month = ("0" + (date.getMonth()+1)).slice(-2),
    ...
    invoice.find({invoiceDate:date},(error, result)=>{
    if(error)
      res.redirect('/error')
    else{
      let finalResult=[]
      if(result.length!==0){
        if(req.body.section!=='' && req.body.brand!=='' && req.body.model!==''){
          let product=[]
          let filteredResult=[]
          for(var i=0; i<result.length; i++){
            for(var j=0; j<result[i].boughtProductsDetails.length;j++){
              if(result[i].boughtProductsDetails[j].section===req.body.section && result[i].boughtProductsDetails[j].brand===req.body.brand && result[i].boughtProductsDetails[j].model===req.body.model){
                filteredResult.push({mrp:result[i].boughtProductsDetails[j].mrp})
                product.push(result[i].boughtProductsDetails[j].brand+' - '+result[i].boughtProductsDetails[j].model+' - '+result[i].boughtProductsDetails[j].mrp+'<br/>')
              }
            }
          }
          let sum=0
        ...
        else if(req.body.section!=='' && req.body.brand!=='' && req.body.model===''){
          let sectionedResult=[]
          let product=[]
          for(var i=0; i<result.length; i++){
            for(var j=0; j<result[i].boughtProductsDetails.length;j++){
          ...
          for(var i=0; i<result.length; i++){
            for(var j=0; j<result[i].boughtProductsDetails.length;j++){
              if(result[i].boughtProductsDetails[j].brand===req.body.brand && result[i].boughtProductsDetails[j].model===req.body.model){
                sectionedResult.push({mrp:result[i].boughtProductsDetails[j].mrp})
                product.push(result[i].boughtProductsDetails[j].brand+' - '+result[i].boughtProductsDetails[j].model+' - '+result[i].boughtProductsDetails[j].mrp+'<br/>')
              }
            }
